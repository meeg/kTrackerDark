...
{
  int ndf;
  if(stationID == 6 && jobOpts->m_enableKMag)
  {
    ndf = getNHits() - 5;
  }
  else
...
{
  if(jobOpts->m_enableKMag && stationID >= 5 && z < Z_KMAG_BEND - 1.)
  {
...
{
  double z = geomSvc->getPlanePosition(detectorID);

  double x_exp = getExpPositionX(z);
  double y_exp = getExpPositionY(z);

  return geomSvc->getCostheta(detectorID)*x_exp + geomSvc->getSintheta(detectorID)*y_exp;
}
...
double Tracklet::getMomentum() const
{
  ...
  //if(!jobOpts->m_enableKMag) return 1E8;
...
double Tracklet::calcChisq()
{
  ...
  for(std::list<SignedHit>::const_iterator iter = hits.begin(); iter != hits.end(); ++iter)
  {
    ...
    double costheta = geomSvc->getCostheta(detectorID); 
    double sintheta = geomSvc->getSintheta(detectorID); 
    double z = geomSvc->getPlanePosition(detectorID);

    double sigma = -999;
    if(iter->sign != 0) 
      sigma = geomSvc->getPlaneResolution(detectorID);
    else
    {
      if( jobOpts->m_coarseMode )
        sigma = geomSvc->getPlaneSpacing(detectorID)/sqrt(12.);
      else
        sigma = fabs(iter->hit.driftDistance);
    }

    double p = iter->hit.pos + iter->sign*fabs(iter->hit.driftDistance);
    int index = detectorID - 1;
    if(jobOpts->m_enableKMag && stationID == 6 && detectorID <= 6)
    {
...
double Tracklet::Eval(const double* par)
{
  ...
  x0 = par[2];
  y0 = par[3];
  if(jobOpts->m_enableKMag) invP = par[4];
...
SRecTrack Tracklet::getSRecTrack()
{
  ...
  for(std::list<SignedHit>::iterator iter = hits.begin(); iter != hits.end(); ++iter)
  {
    ...
    double z = geomSvc->getPlanePosition(iter->hit.detectorID);
