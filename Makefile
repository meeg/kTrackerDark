...
DATAREADERO   = sqlDataReader.o
DATAREADER    = sqlDataReader

MCREADERO     = sqlMCReader.o
MCREADER      = sqlMCReader

UPLOADERO     = sqlResWriter.o
...
CLASSOBJS     = $(GEOMSVCO) $(EVENTREDUCERO) $(JOBOPTSSVCO) $(SRAWEVENTO) $(SRECEVENTO) $(KALMANUTILO) $(KALMANFILTERO) $(KALMANTRACKO) $(KALMANFITTERO) $(VERTEXFITO) \
		$(KALMANFASTO) $(FASTTRACKLETO) $(MYSQLSVCO) $(TRIGGERROADO) $(TRIGGERANALYZERO)
ALIGNOBJS     = $(SMPUTILO) $(SMILLEPEDEO) $(MILLEPEDEO)
OBJS          = $(CLASSOBJS) $(ALIGNOBJS) $(KVERTEXO) $(KFASTTRACKO) $(MILLEALIGNO) $(KTRACKERO) $(DATAREADERO) $(MCREADERO) $(UPLOADERO)
SLIBS         = $(KTRACKERSO)
PROGRAMS      = $(KVERTEX) $(MILLEALIGN) $(KFASTTRACK) $(KTRACKER) $(DATAREADER) $(MCREADER) $(UPLOADER)
...
$(DATAREADER):   $(DATAREADERO) $(CLASSOBJS) $(TRKEXTOBJS)
	$(LD) $^ -o $@ $(LDFLAGS)
	@echo "$@ done."

$(MCREADER):   $(MCREADERO) $(CLASSOBJS) $(TRKEXTOBJS)
	$(LD) $^ -o $@ $(LDFLAGS)
	@echo "$@ done."

$(UPLOADER):   $(UPLOADERO) $(CLASSOBJS) $(TRKEXTOBJS)
	$(LD) $^ -o $@ $(LDFLAGS)
	@echo "$@ done."
